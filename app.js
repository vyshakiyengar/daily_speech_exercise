...
function steps(session) {
 ...
 {title:'Wake up your words.',short:'Loosen lips & tongue',phase:'03 / WARM UP',instruction:'Say each sound slowly and comfortably. Give your lips and tongue time to move. Work down the list, then repeat.',label:'SLOW FIRST. KEEP IT EASY.',phrases:['oo — ee · oo — ee','la — la — la','ta — ta — ta','ka — ka — ka'],tip:'Use small, comfortable movements. If anything feels tense, make the movement smaller and slow down.'},
 {title:'Clear beats. Easy pace.',short:'Make each word clear',phase:'04 / FIND YOUR FLOW',instruction:'Say each phrase once slowly, then again at your everyday pace. Let the last word finish before the next phrase begins.',label:'ONE PHRASE AT A TIME',phrases:['A bright blue morning.','Take a little time.','Pack the paper bag.','Keep the conversation going.'],tip:'Clarity comes before speed here. Don’t race or force consonants. Repeat a phrase if you would like another try.'},
 {title:'Let your sentences breathe.',short:'Read with room',phase:'05 / FIND YOUR FLOW',instruction:'Read this out loud. Take a small pause at each full stop. Read it again at a comfortable conversational pace.',label:'A FRESH PASSAGE FOR TODAY',quote:passages[daySeed(session.date)%passages.length],tip:'Imagine you’re telling one person something interesting. Aim for an easy pace you could use in a real conversation.'},
...
function renderHome() {
 document.querySelectorAll('[data-minutes]').forEach(button => button.setAttribute('aria-pressed',Number(button.dataset.minutes)===state.minutes));
 document.querySelectorAll('[data-intent]').forEach(button => button.setAttribute('aria-pressed',button.dataset.intent===state.intent));
 $('start').replaceChildren(document.createTextNode(state.session ? 'Pick up where you left off' : completed(dateKey()) ? 'Another little warm-up' : 'Let’s warm up'));
...
function start(fresh=false) {
 ...
 if(fresh || !state.session) state.session={version:1,date:dateKey(),minutes:state.minutes,intent:state.intent,index:0,remaining:30,elapsed:0,started:false};
...
function renderStep() {
 ...
 $('back').disabled=session.index===0; $('next').textContent=session.index===6?'Finish practice →':'Done early →'; $('next-up').textContent=session.index<6?`Up next: ${list[session.index+1].short}`:'Next: your personal check-in';
 $('next').disabled=!session.started;
 $('listen').disabled=!('speechSynthesis' in window);$('listen').setAttribute('aria-pressed','false'); updateTimer();
 $('play').textContent=session.remaining<=0?'Next exercise →':session.started?'Resume exercise':'Begin exercise';
 $('timer-state').textContent=session.remaining<=0?'Step done. Take a breath, then continue.':session.started?'Paused. Pick up at your own pace.':'Read the cue. Start when you’re ready.';
...
function pause(){syncTime();stop();save();$('play').textContent='Resume exercise';$('timer-state').textContent='Paused. Pick up at your own pace.';}
function toggle(){if(running){pause();return;}const session=state.session;if(session.remaining<=0){advance();return;}if('speechSynthesis' in window)speechSynthesis.cancel();session.started=true;$('next').disabled=false;running=true;endAt=Date.now()+session.remaining*1000;lastTick=Date.now();$('play').textContent='Pause';$('timer-state').textContent='Your time. Keep it comfortable.';hold();save();interval=setInterval(()=>{syncTime();save();if(session.remaining<=0){stop();$('play').textContent=session.index===6?'Finish practice →':'Next exercise →';$('timer-state').textContent='Step done. Take a breath, then continue.';save();}},200);}
function advance(){if(!state.session.started)return;syncTime();stop();track('exercise_complete',{step:state.session.index+1,minutes:state.session.minutes,finished_early:state.session.remaining>0});if(state.session.index===6){finish();return;}state.session.index++;state.session.remaining=steps(state.session)[state.session.index].seconds;state.session.started=false;save();renderStep();}
function finish(){const session=state.session;track('session_complete',{minutes:session.minutes,elapsed_seconds:Math.round(session.elapsed||0)}); const tomorrow=new Date();tomorrow.setDate(tomorrow.getDate()+1);$('tomorrow-preview').textContent='A fresh reading prompt is waiting: “'+passages[daySeed(dateKey(tomorrow))%passages.length].split('. ')[0]+'.”';lastCompleted=dateKey(); const previous=state.days[lastCompleted]||{};state.days[lastCompleted]={...previous,complete:true,minutes:session.minutes,elapsed:Math.round(session.elapsed||0),count:(previous.count||0)+1};state.session=null;save();show('done');$('done-summary').textContent='Seven small steps. One real sentence to take into your day.';renderFeeling();$('share-status').textContent='';$('done').querySelector('h1').setAttribute('tabindex','-1');$('done').querySelector('h1').focus({preventScroll:true});}
function renderFeeling(){const feeling=state.days[lastCompleted]?.feeling;document.querySelectorAll('[data-feeling]').forEach(button=>button.setAttribute('aria-pressed',button.dataset.feeling===feeling));$('feedback').textContent=({easier:'Keep the cue that helped. Try it in your next conversation.',same:'That’s a useful observation too. Practice doesn’t need to feel dramatic to count.',harder:'Thanks for noticing. Take a break and keep the next conversation comfortable.'})[feeling]||'Notice your pace, the space between words, or how comfortable you feel.';}
